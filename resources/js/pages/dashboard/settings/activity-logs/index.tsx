...
import { ActivityLogType, QueryResultType, SharedData } from "@/types";
import { usePage } from "@inertiajs/react";
import { Button, Descriptions, Dropdown, Modal, Typography } from "antd";
...
export default function Index({ queryResult }: IndexType) {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "created_at",
      render: (created_at: string) => dayjs(created_at).format("dddd, DD MMM YYYY | HH:mm:ss"),
    },

    {
      ...
      align: "center",
      width: 1,
      render: (id: string, record: ActivityLogType) => (
        <Dropdown
          trigger={["click"]}
  ...
  const items = [
    ...
    {
      key: "actor",
      label: __(locale, "lang.actor"),
      children: selectedData?.causer?.name,
    },
    {
      key: "actor_type",
      label: __(locale, "lang.actor_type"),
  ...
  return (
    <DashboardLayout
      ...
      extra={<InputSearchDatatable queryResult={queryResult} route={route("settings.activity-logs.index")} />}
    >
      <Datatable queryResult={queryResult} route={route("settings.activity-logs.index")} columns={columns} />
